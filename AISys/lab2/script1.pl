...
get_item((Item, _), Item).

find_available_crafts(Inventory, Crafts) :-
 findall(Item, (
 recipe(Item, Ingredients, _),
 check_ingredients(Ingredients, Inventory)
 ), Crafts).

check_ingredients([], _).
check_ingredients([(Item, Quantity) | Tail], Inventory) :-
 (
 member((Item, Available), Inventory),
 Available >= Quantity
 ),
 check_ingredients(Tail, Inventory).

% Проверка наличия ингредиентов (как и раньше)
has_ingredients(Item, Quantity) :-
    inventory(Items),
    member((Item, Available), Items),
    Available >= Quantity.
...
% Функция для подсчета максимального количества создаваемых предметов
max_creatable(Item, MaxCount) :-
    recipe(Item, Ingredients, _),
    findall(Max, (
        member((Ingredient, Quantity), Ingredients),
        has_available(Ingredient, Quantity, Max)
    ), MaxList),
    MaxList \= [],
    min_list(MaxList, MaxCount).

% Проверка наличия ингредиентов и вычисление доступного количества
has_available(Item, Quantity, Max) :-
    inventory(Items),
    member((Item, Available), Items),
    Max is Available // Quantity.
