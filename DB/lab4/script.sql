...
select  "Н_ЛЮДИ"."ИД" as isu,
    ...
    where (
            ...
            where "Н_ФОРМЫ_ОБУЧЕНИЯ"."НАИМЕНОВАНИЕ" = 'Заочная'
                ...
                and "Н_КВАЛИФИКАЦИИ"."НАИМЕНОВАНИЕ" like '%Педагог%'
        ) = 10
    ;
...
select DISTINCT on ("ФИО")
    ...
    where "Н_УЧЕНИКИ"."ГРУППА" = '3100'
        and not exists(
            ...
            where "Н_ЛЮДИ"."ИД" = "Н_ВЕДОМОСТИ"."ЧЛВК_ИД"
                and "Н_ОЦЕНКИ"."СОРТ" not in (1,2,5))
        and exists(
            select 1 from "Н_ВЕДОМОСТИ"
                inner join "Н_ОЦЕНКИ" on "Н_ОЦЕНКИ"."КОД" = "Н_ВЕДОМОСТИ"."ОЦЕНКА"
            where "Н_ЛЮДИ"."ИД" = "Н_ВЕДОМОСТИ"."ЧЛВК_ИД"
